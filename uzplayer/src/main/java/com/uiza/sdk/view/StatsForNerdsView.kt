...
public class StatsForNerdsView extends RelativeLayout implements AnalyticsListener, OnAudioVolumeChangedListener {
    ...
    private void init() {
        ...
        // close button
        findViewById(R.id.btn_close).setOnClickListener(v -> UZViewUtils.goneViews(StatsForNerdsView.this));
    }
...
//        Timber.e( "totalBufferedDurationMs: %ld", eventTime.totalBufferedDurationMs);
        mHandler.post(() -> {
            ...
            if (bitrateEstimate < 1e6) {
                formattedValue = getResources().getString(R.string.format_connection_speed_k,
                        StringUtils.doubleFormatted((bitrateEstimate / Math.pow(10, 3)), 2));
            } else {
                formattedValue = getResources().getString(R.string.format_connection_speed_m,
                        StringUtils.doubleFormatted((bitrateEstimate / Math.pow(10, 6)), 2));
            }
            textConnectionSpeed.setText(formattedValue);
            textNetworkActivity.setText(StringUtils.humanReadableByteCount(totalBytesLoaded, true, false));
            textBufferHealth.setText(getResources().getString(R.string.format_buffer_health, StringUtils.doubleFormatted((eventTime.totalBufferedDurationMs / Math.pow(10, 3)), 1)));
        });
    }
    ...
    private void depictVideoInfo() {
        ...
        setTextHost(UZData.getInstance().getHost());
    }
    ...
    public void hideTextLiveStreamLatency() {
        UZViewUtils.goneViews(textLiveStreamLatency, textLiveStreamLatencyTitle);
    }
    ...
    public void showTextLiveStreamLatency() {
        UZViewUtils.visibleViews(textLiveStreamLatency, textLiveStreamLatencyTitle);
    }
}
